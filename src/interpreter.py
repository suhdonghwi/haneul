...
class BytecodeInterpreter:
  def __init__(self, const_table, var_names, code, parent=None):
    ...
    self.stack = []
    self.local_vars = [None] * len(self.var_names)
  ...
  def find_index(self, index):
    ...
    if value is None:
      if self.parent is None:
        ...
        return self.parent.find_name(self.var_names[index])
    return value
  ...
  def run(self):
    ...
    for inst in code_iter:
      if inst.opcode == INST_PUSH:
        ...
        print "STORE"
        self.local_vars[inst.operand] = self.stack.pop()
        ...
        for (i, arg) in enumerate(args):
          func_interpreter.local_vars[i] = arg
